...
@section('title', 'Chỉnh sửa đặt sân - ' . $booking->court->name)
            ...
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title mb-0">
                        <i class="fas fa-edit me-2"></i>Chỉnh sửa đặt sân: {{ $booking->court->name }}
                    </h4>
                </div>
                <div class="card-body">
                    <!-- Important Notice -->
                    <div class="alert alert-info">
                        ...
                        <ul class="mb-0">
                            <li>Bạn chỉ có thể chỉnh sửa đặt sân khi chưa thanh toán</li>
                            <li>Không thể chỉnh sửa đặt sân đã bắt đầu hoặc đã kết thúc</li>
                            <li>Sau khi chỉnh sửa, giá có thể thay đổi và cần thanh toán lại</li>
                        </ul>
                    </div>
                        ...
                        <div class="col-md-6">
                            <div class="card bg-light">
                                <div class="card-body">
                                    ...
                                    <p class="mb-1"><strong>Tên sân:</strong> {{ $booking->court->name }}</p>
                                    <p class="mb-1"><strong>Vị trí:</strong> {{ $booking->court->location }}</p>
                                    <p class="mb-1"><strong>Giá:</strong> {{ number_format($booking->court->price_per_hour) }} VND/giờ</p>
                    ...
                    <!-- Booking Form -->
                    <form method="POST" action="{{ route('user.bookings.update', $booking->id) }}" id="bookingForm">
                        @csrf
                        @method('PUT')
                                ...
                                <select class="form-control @error('start_time') is-invalid @enderror"
                                    ...
                                    @for($hour = 6; $hour <= 22; $hour++)
                                        <option value="{{ sprintf('%02d:00', $hour) }}"
                                                {{ old('start_time', $booking->start_time) == sprintf('%02d:00', $hour) ? 'selected' : '' }}>
                                            {{ sprintf('%02d:00', $hour) }}
                                        </option>
                                    @endfor
                                </select>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select class="form-control @error('end_time') is-invalid @enderror"
                                    ...
                                    @for($hour = 7; $hour <= 23; $hour++)
                                        <option value="{{ sprintf('%02d:00', $hour) }}"
                                                {{ old('end_time', $booking->end_time) == sprintf('%02d:00', $hour) ? 'selected' : '' }}>
                                            {{ sprintf('%02d:00', $hour) }}
                                        </option>
                                    @endfor
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror"
                                      ...
                                      placeholder="Nhập ghi chú cho đặt sân...">{{ old('notes', $booking->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary" id="submitBtn">
                                <i class="fas fa-save me-1"></i>Cập nhật đặt sân
                            </button>
                        </div>
                    </form>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const pricePerHour = {{ $booking->court->price_per_hour }};
